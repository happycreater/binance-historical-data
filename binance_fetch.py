...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Optional arguments:\n"
            "  --symbols         Omit to fetch all symbols; supports wildcards like '*USDT'.\n"
            "  --intervals       Required for klines/indexPriceKlines/markPriceKlines/premiumIndexKlines.\n"
            "  --output-path     Root directory for downloads (default: data.binance.vision).\n"
            "  --parallel        Concurrent downloads (default: 5).\n"
            "  --no-validate-params  Skip parameter validation.\n"
            "  --api-proxy       Proxy for Binance API symbol discovery (e.g., http://localhost:7890)."
        ),
...
def append_missing(output_path: Path, url_path: str, reason: str) -> None:
    ...
    with missing_path.open("a", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow([url_path, reason, dt.datetime.utcnow().isoformat()])
